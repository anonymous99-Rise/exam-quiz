...
## 七、音频代理（/api/media）与「直连失败」的真实原因

用户反馈「这一集的音频没取到」。实测把因果分清了：

| 环境 | 直连 VOA/BBC 音频 | 结论 |
| --- | --- | --- |
| 本机（Clash 开系统代理，浏览器能走） | ✅ `readyState 4` 正常播放 | 前端状态没问题 |
| 用户浏览器（规则未覆盖这些域名 / 线路不通） | ❌ 报错 | **必须服务端兜底** |

所以播放器加了**两级**：直连 → 失败自动切 `/api/media?u=`（本站代理，浏览器只跟本站说话）。
播放条会显示「代理线路」小签，用户知道当前走的是哪条路。

代理的两个工程细节（不做会出事）：

1. **Range 分片 + 单次上限 3.5MB**。播客单集 20–28MB，而 serverless 响应体有 4.5MB 上限。
   浏览器拖动进度本来就在发 Range，所以把**开区间** `bytes=0-` 改写成
   `bytes=0-3499999` 并如实返回 206 —— 实测 BBC 返回
   `206 content-range: bytes 0-3499999/3692473`，客户端会自然续下一片。
2. **主机后缀白名单**（`voanews.eu` / `bbci.co.uk` / `acast.com` / `archive.org` …），
   否则这就是一台公开代理。

## 八、正文提取（/api/article）：能抓就抓，抓不到就说清楚

用户在阅读页要「全文」。实测三个站点的真实边界（这决定了不能只有一条路）：

| 站点 | 情况 | 结果 |
| --- | --- | --- |
| ScienceDaily | 正文在静态 HTML 里 | ✅ 抓到 6 段 / 2510 字 |
| Nature / Science.org | Cloudflare 挑战页（3KB `Client Challenge`）+ 正文付费 | ❌ 如实失败 |
| VOA 节目页 | 文稿是**前端动态渲染**的（初始 HTML 只有 4 段） | ❌ 只能拿到 228 字页面说明 |

实现：`src/lib/feeds/extract.ts` 是一个**轻量容器栈扫描器**——顺序扫标签、维护
div/section/article 路径、把段落归到路径上，再按「非导航类 + 有正文类关键词 + 字数多」
挑最优容器。不引 DOM 解析库（那要连带 jsdom，几百 KB + 构建风险）。

关键约束：**`MIN_USEFUL_CHARS = 600`**。抓到的字太少（如 VOA 那 228 字）不算成功 ——
否则用户会以为「文稿就这么点」。低于阈值一律按失败处理，UI 给出官网入口。

播放器的「文稿」面板也走这条链路，分两层：**订阅源自带的节目说明**（一定有，离线也有）+
**按需抓取的节目页正文**（抓不到就说明「该节目文稿是动态加载的」，不硬凑）。
...
- **逐句跟读 / 时间轴对齐**：文稿面板已上线（订阅源说明 + 按需抓取），但**还没有时间戳**
  —— 真正的「跟读高亮」需要音频时间轴与文本对齐；VOA/TED 的文稿多为前端渲染，
  可行路线是接它们的官方 transcript 接口，或对音频做本地 ASR（成本高，暂不做）。
